...
用户报：重载前后开了两个以上的编辑区标签，恢复出来的**会话与标签对不上**（标签 1 原本是
会话 A、标签 2 是 B，重载后互换）。

- **根因是恢复只认顺序**：工作区缓存里记的是「按创建顺序的第 N 条」（`activeOrder`），
  而 VS Code 恢复的是**上次的编辑器排布**——两者不保证一致，对不上位就张冠李戴。
- **修法（身份优先，顺序兜底）**：界面把「这个窗口开着哪条会话」写进 VS Code 的 webview
  state（`acquireVsCodeApi().setState()`，这是宿主唯一读得到的持久身份），
  `deserializeWebviewPanel(panel, state)` 里读回来按身份认领会话
  （`WindowRestore.classifyPanel`）。旧版本写的面板、状态对不上缓存、同一会话开在两个
  窗口——这些情况自动退回原来的按顺序认领，不会把窗口留成空态。
- 写缓存也跟着改成**按条问「这个下标认领过没有」**（`mergeWindowCache.panelClaimed`）：
  按身份认领时下标是跳着的，拿「认领了几条」当游标会把还没露面的那条丢掉。
- **标签标题**：会话有标题就显示标题（不再恒写 `DSH`），还没有标题的新会话显示 `DSH`；
  标签保持**纯静态**，不带运行状态、图标也不随状态变（「在不在生成」由会话界面自身表达）。
- 认领与恢复各留一行日志（`[restore] 面板 … 认领：按身份/按顺序（webview 存的是 …）→ …`），
  下次再出问题在输出通道里能直接看出是身份没读到还是缓存本身错了。
...
用户报：生成过程中离开这个会话（历史抽屉点别的会话，或重载 VS Code 窗口），回来时那一轮
**分裂成两条思考节点**——前一条只剩一个字的思考、还一直显示在跑（实际早已停），后一条才是
完整思考；等这一轮跑完再打开该会话却一切正常。

- **根因是漏读了一条契约**（已补记进 `docs/dsh-server-api.md` §6.7）：为**已经在跑的
  attempt** 重开 follow 时，服务端**不重发 `start` 帧、也不重发已经发过的增量**，进行中的
  内容只在开帧的 `snapshot.assistantStream.activeAttempt`（`{attemptId, turn, step,
  nextIndex, stream}`）里。适配器此前只声明了这个字段、从不消费它。
- 由此三件事一起发生：叠加层没有 turn/step 归属（durable 结算时匹配不上 → **另推一条**，
  旧的那条留着且 `streaming` 永不清）；正文只有挂上之后收到的增量（折叠摘要取最后一行，
  看着就是**一个字**）；只发帧不改模型，所以任何一次整条下发都会把界面已经长好的节点
  打回第一个增量。
- **修法**：新增 `src/dsh/assistantStream.ts`（官方 `expandAssistantStream` 的等价实现，
  把紧凑记录展开回 timed chunk）；适配器在开帧后、`messages/reset` 前用
  `replayActiveAttempt` 把基线折回模型（静默，不逐条发帧）并认领 attempt 的 turn/step；
  增量落到模型上（`appendLiveDelta`）；认不出归属的叠加层按通配就地认领。
- 断言：`scripts/thinkingStream.test.ts` A3d（展开器逐类 + 坏记录跳过）、A4（带基线的开帧 +
  无 `start` 的增量 + durable 结算，改前红：两条节点、`streaming` 未清）、A4b（基线缺失也
  不分裂）、A4c（截断窗口里「这一轮在跑」由基线补回）。
- 无用户可见文案变化，无 i18n 变更。

### 界面：工具结果里的图片改为「展开才渲染，收起不渲染」（2026-09-21）

用户口径：读取图片这类节点，**展开了才渲染图片，收缩态一张都不渲染**。这条**取代**
2026-09-18 的相反口径（当时为了「agent 发来的图折叠态也看得见」，把图挪到工具行**下方**、
不再算展开体成员）。

- **改法**（`src/webview/components/Rows.tsx`）：图库挪回 `Row` 的 children——`Row` 只在展开时
  渲染 children，所以「收起不渲染」由结构本身保证，不靠额外条件判断；收起态连 `<img>` 都
  不存在，不再替一个看不见的盒子解码图片（`read_image` 多的会话里这是几十张）。
  展开态照旧是同一套 `ImageGallery`，点缩略图仍是原图浮层。
- **图片同时算进 `hasBody`**：否则「只有图、没有别的正文」的调用会变成一行**点不开**的节点
  ——图渲染得出、用户却永远打不开，是这条改动最容易漏掉的一半，断言专门钉了它。
- **范围是所有工具节点的图片结果**（`tool.images`：`read_image`、截图等任何工具回带的 image
  块），不按工具名特判。用户自己发出的图、助手消息里的 `images` 段、交付 / 生成的图片文件
  都不受影响（它们本来就不在可折叠节点里）。
- 断言：`scripts/imageRender.test.ts` §6 由「折叠态就看得见」改为双向钉（收起态不许出现
  `<img>` / `row-body-images`、展开态必须画出来且图库在行头之下），新增 §6b 钉源码里
  「图片算进 `hasBody`」。
- 预览页实测（Playwright 驱动 `test/preview.html`，零 token）：折叠态该行 `img` 数为 0，
  点开后为 1（`aria-expanded` 同步翻转），再收起回到 0。
- 无用户可见文案变化，无 i18n 变更。

### 修复：思考 / 正文节点在「流式 → durable」时被重挂，用户展开的节点自己收回去（2026-09-21）

用户报「思考节点生成中展开、跑完会自己收缩回去，普通工具节点却不会」，并问是不是
「工具跑得太快所以看着一样」的错觉。

- **不是错觉，是真不一样**：段落的 `id` 就是界面上那条节点的 React key，而两类节点的
  id 稳定性不同——工具行的段 id 恒为 `tool:<callId>`（流式 → durable → 结果全程不变），
  思考 / 正文的**流式叠加层**却叫 `live:<attempt>:<index>`，durable 段另叫 `r<seq>:<n>` /
  `t<seq>:<n>`。durable 一到，旧实现是「删掉叠加层（且先单独发一帧 upsert）+ 推一条新 id
  的段」，于是组件被重挂：用户手动展开的状态、盒子里的滚动位置、正在划的选区一起丢。
- **修法**（`src/dsh/adapter.ts`）：durable 内容**就地接管**流式叠加层——段 id 不动，只把
  正文换成权威版本、收掉 `streaming`，一帧 upsert 走完，中途不会出现「这条段不存在」的提交。
  两条边界保持老行为：跨消息段的叠加层（运行中插话把轮切开、叠加层留在旧段）仍「摘掉 +
  推新段」，否则 durable 内容会留在旧段、跑到插话**上方**；durable 里没有对应块的叠加层照旧
  摘掉（否则留下一条永远停在半截的节点）。另加一道防重复：durable 接管后**迟到**的增量帧
  丢掉，不再造出一条与它同 id 的段。
- **展开态改由消息持有**（新增 `src/webview/nodeOpen.ts`）：思考 / 工具 / 上下文注入 /
  命令 / 未知块这五类可折行的展开态从各行自己的 `useState` 提到 `Message` 上，按段 id 记，
  行只读一个端口对象（`{ open, openedWhileActive, setOpen }`）。这样过程折叠把整段成员
  卸载再挂回来时展开态不丢——此前「生成中点开 → 一轮结束被折叠 → 再点开按钮」那个节点
  会变回收起态。
- **过程折叠不再折掉用户点开的节点**（用户 2026-09-21 口径）：这一轮里凑够阈值的那一段，
  只要其中有**用户点开且还开着**的节点，就默认**保持展开**（不然那个节点会跟着被卸载）；
  顶端的折叠按钮照旧在，想收起这一整段随时可以点，点过之后就以用户的选择为准。
- **跑完那一刻新出现的正文盒子按展开时的状态定位**（用户第 2 条口径）：工具行跑的时候是
  「运行状态块 + 输入参数卡」，跑完换成结果卡 / 终端卡——那是一个**新元素**，它的滚动位置
  此前天然是顶部。现在记下「用户是**在它还跑着的时候**点开的」：这种就贴底（延续运行中的
  「跟着最新一行」）；结束后才点开的照旧置顶。`useStickyBody` 因此多了第 4 个参数
  `openedWhileActive`（默认取 `active`，旧调用点行为不变）。
- 断言：`scripts/thinkingStream.test.ts` 新增三段（段 id 稳定 + 思考仍在工具行之前 /
  跨段不就地接管 / 多余与迟到的叠加层不造重复 id）；新增 `scripts/nodeOpen.test.ts`
  （状态纯逻辑 + 端口读写 + 接线：状态在消息里、行里只读端口、折叠默认展开判据、按钮保留）；
  `scripts/styles.test.ts` §16c 与 §3c 随形态更新（第 4 个参数、依赖项、行不再自持 useState）。
- 预览页实测（Playwright 驱动 `test/preview.html`，零 token）：① 展开中的流式思考节点在
  「换 id」那两帧下会收缩（复现旧行为），在「同 id」那帧下保持展开、`scrollTop` 与 DOM 节点
  都不变；② 生成中点开的节点在一轮结束时，那一段过程**默认就是展开的**（按钮 `aria-expanded=true`），
  手动收起再展开它仍是展开的；③ 运行中点开的工具行跑完换卡后 `scrollTop` 落在底部
  （830−320=510），收起再打开（= 结束后才打开）回到顶部。
- 无用户可见文案变化，无 i18n 变更。
...
- **会话页任意位置 `Ctrl+V`，剪贴板里的图片（截图）与文件（资源管理器里复制的）直接进
  附件栏**。粘贴有两条落点：剪贴板里是**文件/目录**时宿主取真路径、走与「添加文件」完全
  同一条路（见下方用户反馈那一节）；剪贴板里不是文件时（截图、复制的图片数据）读成字节走
  `attachBytes`，与拖放同一个字节通道。**纯文本一律放行**给 textarea 的原生插入——
  只有认出文件才 `preventDefault()`。
- 接取逻辑从 `src/webview/dropAttach.ts` 迁到 **`src/webview/attachIntake.ts`**（文件本来就叫
  drop、现在两条路都管），常量 `DROP_BYTES_LIMIT` 随之改名 `ATTACH_BYTES_LIMIT`（界面 + 宿主
  两处同值同义）；帧上多了 `source: "drop" | "paste"`，宿主按它分两套提示措辞
  （「拖放上限 8 MB」对粘贴来的东西说不通，用户手上没有那个可拖的文件），缺省当拖放。
- **为什么粘贴能在 webview 里接到**（本机 VS Code `resources/app/out` 逐行读过，别再怀疑
  「webview 收不到粘贴」）：桌面版 `pre/index.html` 的 `handleInnerKeydown` 对 `Ctrl+C/V/X`
  一律 `preventDefault()`（原生粘贴被掐掉），再把按键交给宿主；宿主 `WebviewElement.handleKeyEvent`
  把它重派到主窗口、键位解析到 `paste` → `WebviewElement.paste()` → `_send("execCommand","paste")`；
  pre 脚本收到后对内容文档执行 `execCommand("paste")`（iframe 的 `allow="clipboard-read; clipboard-write"`
  正是这条权限）——于是 **paste 事件照常到 webview**。所以**不要**自己挂 keydown 抢 `Ctrl+V`。
- 无名截图补一个带后缀的名字（`pasted-image.png`）：宿主的图片判据只看文件名后缀，
  没后缀会退化成「普通文件上传」。
- 断言：`scripts/attachments.test.ts`（真调用界面模块 + 假 DataTransfer：纯文本放行 /
  `files` 与 `items` 两条来源 / 无名补后缀 / `source` 与 base64 / 超限不读只报名字 /
  宿主分派两套标记）、`scripts/i18n.test.ts`（两条新标记有发射点、两种语言可解析）。
- 空态提示补一句「也可以直接粘贴图片或文件」——这条路没有按钮可点，不公告就等于不存在。

**用户随即报的三个问题 + 一条口径（同一改动，未单独发布）**：

- **「粘贴什么都变成 png」**：`namedClipboardEntry` 上一版写的是
  `IMAGE_EXTENSIONS[mime] ?? "png"`——任何无后缀的名字都被补成 `.png`。实测（VS Code 1.138
  真实剪贴板）：复制目录 `docs` 得到 `File{name:"docs",type:"",size:0}`、复制无扩展名文件
  `LICENSE` 得到 `File{name:"LICENSE",type:"",size:2001}`，两者都被改名叫 `*.png`。**修法**：
  只有 MIME 明确说是图片才补后缀，其余保留原名。
- **目录被当成 0 字节附件**：复制目录时 Chromium 给的是一个 0 字节、`type=""` 的 File，
  读它的字节要么抛「A requested file or directory could not be found…」、要么给空串。
  现在 0 字节条目**不读**、直接报 `unreadable`（宿主提示「目录用 @ 引用即可」）。
- **粘贴大文件被 8 MB 挡住，和「添加文件」不一样**（用户口径④：粘贴要与附件上传同一条路：
  目录 → 路径、图片 → 按图片处理、文件 → 同样是上传）。根因是 webview **拿不到路径**——
  实测粘贴的 `types` 只有 `["Files"]`，`text/uri-list` 与 `text/plain` **都是空串**。
  **修法**：宿主侧新增 `src/dsh/clipboardPaths.ts`，粘贴时先从**系统剪贴板**读
  「文件/目录」列表（Windows：`powershell.exe -Sta` 跑 WinForms `Clipboard.GetFileDropList()`；
  显式设 UTF-8 输出，否则中文路径乱码；5 秒超时、`windowsHide`；非 Windows 或读不到就返回空），
  取到路径就按**类别分流**（见下一条）；取不到才退回字节通道（截图那类剪贴板里本来就没有
  文件的形态）。先后顺序不可反。
- **目录必须是 `@dir/` 引用，不是附件**（用户第二轮口径：粘贴文件夹「还是变成附件形式而不是
  @ 引用」）：拿到真路径后**目录走 `insertMention`**（`@目录/` 写进正文，与资源管理器右键
  文件夹、命令面板「添加文件夹」同一条路），**文件走附件通道**（图片内容块 / 其余不限大小地
  上传）。原先一律走 `addPaths` 是错的——那会把目录落成附件芯片。
- **`@` 列表里点目录 = 进目录，不是选中它**（用户第二轮口径）：
  行主体点击改为 `clickAction(shape)`（目录 = 下钻、其余 = 选中），行右侧「整个目录」按钮
  保持选中整个目录——此前两处都是 `pick`，那枚按钮等于多余。键盘分工**不动**（Enter 引用、
  Tab 进目录，与官方一致），于是鼠标与键盘各有一套路：想引用整个目录，鼠标点按钮、键盘按 Enter。
  行主体的悬停提示也补成「进入目录」。实测（预览页 + Playwright）：点主体 → 正文变
  `@src/webview/` 且弹层留着（下钻）；点尾部按钮 → 同样正文、弹层关闭（选中）。
- **「添加为上下文」这句不准确**（用户口径①）：文件选择器的确认按钮原来写
  `Add as context` / 「添加为上下文」，可那些文件变成的是**附件芯片**（图片内容块 / 上传），
  不是插进正文的引用。现在文件选择器是「添加附件 / Add attachments」；目录选择器确实插的是
  `@dir/` 引用，改为「添加目录引用 / Add folder reference」。
- 断言：`scripts/attachments.test.ts`（只给**图片**补后缀 / 0 字节报 unreadable /
  粘贴按「目录 vs 文件」分流真路径再退回字节通道的顺序）、
  `scripts/mentionNav.test.ts`（`clickAction`：点目录行 = drill、文件 / 对话 / `..` = pick）、
  `scripts/styles.test.ts`（接线：行主体走 `clickAction`、尾部按钮仍是 `pick`）、新增
  `scripts/clipboardPaths.test.ts`（解析 / 脚本形状五条：UTF-8、`-Sta`、`GetFileDropList`、
  `windowsHide`、超时 / 非 Windows 闸门 / **真调用**一次真读取）。
- **打包漏网补一条**：验证时 `npm test` 重定向出来的 `tmp-*.log` 被打进了 vsix（装出来的扩展
  目录里躺着 `tmp-tf.log`）。`.vscodeignore` 原本只排了 `debug.log` 这一个名字，现在按
  **后缀**排 `*.log`，并加上 `tmp-*`——同一类盲区这个文件已经踩过两次（debug.log、四张测试图）。

**第三轮口径（`@` 引用的文本形态，同一改动，未单独发布）**：

- **分隔符统一成 `/`**（原话：「引用进目录时，将 `\` 统一换成 `/`」）：`formatFileMention`
  先把 `\` 归一成 `/`，再补目录尾斜杠——Windows 侧 `path.relative()` 给的是 `\`，
  与服务端候选的 `/` 混在一起就成了 `@src\webview/` 这种半截混合形态。
  `@` 列表的「下钻 / 回上一层」是**直接改正文、不走 token 生成**，所以那两处也调
  `normalizeMentionPath`。**刻意偏离官方**（官方保留调用方给的分隔符、且不加归一）。
  这条对文件同样生效：两处产生方（宿主的引用芯片、界面的候选插入）必须逐字一致。
- **带空格的目录，引号成对**（原话：「引进带空格目录时，只有头部有引号、尾部没有」）：
  官方对目录只写**开**引号（`@"my dir/`，理由是「未闭合的 token 还能继续往下打字」），
  但那个形态一旦发出去就是半截 token。带空格的路径**本来就没法**用触发词继续下钻
  （查询按空白切段），所以官方的理由在这里不成立 → 现在写 `@"my dir/"`（尾斜杠留在
  引号**内**，目录标记不能丢）。同样偏离官方。
- 断言：`scripts/references.test.ts`（逐字：归一 / 成对引号 / 尾斜杠位置 / 绝对路径）、
  `scripts/mentionNav.test.ts` 第 13b 段（钩子层真跑：候选带反斜杠时下钻与 `..` 写回正文的形态、
  选中带空格目录的正文形态）。

### 展开的节点：还在跑的贴底，已结束的置顶（2026-09-21，用户口径）

- 用户原文：「未结束还在执行中的节点打开后，垂直滚动条应该是置底以便能看到最新信息」。
  此前（2026-09-16 口径）一律**置顶**，只有思考节点在「还装得下」时才跟随——于是一段长
  输出、一个长 IN 打开后停在末尾/开头，都要自己手滑。
- 现在 `useStickyBody` 的第三个参数从 `streaming` 换成 **`active`**（这个节点还在执行 /
  内容还在增长）：`active` 为真则**打开即贴底**并持续跟随，为假则**置顶**且不注册观察者。
  接线四处：工具行两个 body（`running`）、思考（`streaming`）、命令节点（`state === "running"`）；
  注入节点（系统提示词等）永远不是「进行中」，保持置顶。
- **取消「盒子装得下才跟随」的限制**：那条限制正是这次报的现象（内容一长就退回顶部）。
  用户的滚动位置仍受保护——上滑即脱跟随，滑回底部恢复（与主对话区同一套粘性判据）。
- **打开那一刻只定位一次**：`active` 由真翻假（节点跑完）时不再重定位，否则用户正盯着
  末尾看输出、节点一结束就被拽回顶部。
- 夹具补两个「进行中且比盒子高」的节点（`r2` 长思考 + `tool8` 长 IN 的运行中工具），
  `npm run preview` 里与 tool6c（40 行输出的**已结束**终端卡）对照着看。
- 断言：`scripts/styles.test.ts` 第 16c 段（两分定位 / 不许再回「一律置顶」/ 不许再有
  「装得下才跟随」/ `openedRef` 只定位一次 / 五处接线）。

### 加载更早的历史改为「按需两档」，取消滚动自动加载（2026-09-20）

- **不再一次取完整个历史**，改为与官方 `dsh` 客户端同构的两档：
  - **单页档**（会话页与轨迹视图的「加载更早」按钮）：点一次取一页 50 条，对应官方
    `ISession.loadOlder()`；
  - **到目标档**（右侧轮次横条上那些「未加载」的刻点）：取到窗口覆盖那一轮的
    `turn/start` seq 为止再落位，对应官方 `ISession.loadThrough(seq)`——「点未加载的
    刻点能先加载再跳转」这个能力保留。
- **取消会话页的滚动自动加载**（原先滚到顶附近会自动取）。官方**会话页**本来就只有
  「加载更早」按钮——`OLDER_LOAD_THRESHOLD_PX` 是官方**轨迹表**的行为。
- 按钮文案随之改为「加载更早的历史 / Load earlier history」（原先写「加载全部历史」，
  早已与实际行为不符）。
- **连取多页只结算一次**：每页只吸收，取完（或中止）重折一次、只发一份
  `messages/reset`。逐页结算会让一次跨轮跳转变成十几次全量重渲染（消息列表没有
  虚拟滚动）。
- **页大小保持 50**，不采用官方的 `JUMP_PAGE_MESSAGES = 200`：单帧上限
  `MAX_LINE_CHARS = 1 MiB` 超限即断流；实测最大会话 20.6 MB（1234 条可分页消息），
  200 条/页约 3.3 MB 必然超限。
- 断言：`scripts/historyReplay.test.ts`（两档判据 / 一次结算幂等 / 无滚动触发）、
  `scripts/turnRail.test.ts`（未加载刻点带目标 seq）。

**用户随后反馈的两处修正**（同一改动，未单独发布）：

- **目录节点点击永远转到用户消息处**：跟随窗口按消息条数切，切点常落在某轮的助手消息
  上，于是**窗口第一轮**的用户消息不在窗口里。此前这类轮次的锚点会**退化成该轮第一条
  助手消息**并被标成「已加载」，点击只落到该轮已加载的最早内容，永远到不了用户消息处
  ——而且这是常态不是边界（一轮里助手消息比用户消息多一个数量级，本机实测 1125 : 109）。
  现在：大纲说这轮有提示词（全日志里确有 `user/message`）而窗口里找不到它时，该刻点
  保持「未加载」呈现（更短更淡），点击走「到目标档」把用户消息取回来再落位；只有确实
  没有更早历史可取（`hasMoreHistory === false`）或大纲说这轮本就没有用户消息（自动轮）
  时才退化。另记 `visibleMessageId` 供命中测试，避免读者滚到该轮时激活轮点亮偏一轮。
- **落位之后继续跟踪到布局稳定**，并且**目录第一枚刻点一律置顶**：落位原本只做一次，
  而**需要加载历史**的那些轮次，目标行**上方**会插入内容（窗口从 `seq=0` 起时，注入 /
  幻影轮都在它上面），其中代码高亮、Markdown、图片、折叠都是**异步变高**的——它们在
  落位**之后**才撑开，把目标行推离阅读线且无人校正（实测：落位 24px → 上方撑开 300px
  → 目标漂到 258px）。已加载轮次不插入任何内容，所以「中间那些」一直是好的。
  两道措施：① 一般轮次落位后**逐帧**盯着目标行（缓存行引用，正常每帧只读一次
  `getBoundingClientRect`），偏差就把它拉回阅读线，观察窗 5 秒，**用户一有滚动手势
  立刻停**（跟踪不跟用户抢方向盘）；② **第一枚刻点不再对齐阅读线**，改为等宿主说
  「取完了」（`historyLoading` 落回 false）就 `scrollTop = 0`——顶部没有还能再变高的东西，
  落完就是稳的。**注意它等的是「取完了」而不是「该轮变成 loaded」**：后一条要绕一圈
  （messages → items → anchor.kind），实战里常常等不到，脉冲空转、视口不动。
  ③ **置顶与落位之前都必须再放一次跟随**：取历史的补偿会让视口落在（近）底部，而
  `scroll()` 的「回到近底部即恢复跟随」会把意愿翻回 `true`，rAF 里的 `settle()` 随即
  `scrollTop = scrollHeight`，把刚算好的位置整个吃掉（实测：置顶到 0 之后又被钉回 5816）。
  这也解释了为什么「再点一次就好了」——第二次走的是已加载分支，那条路径本来就有放跟随。
  中间踩过一个坑：跟踪的第一版收手条件写成「连续 12 帧已对齐就停」（≈200ms），而慢内容
  常在落位后 ~270ms 才撑开，正好漏掉——是诊断日志里的「跟踪停止：帧=12」把它暴露出来的。
  断言 `scripts/turnRail.test.ts`（第 8b 段四条分支 + 命中索引 + 落位跟踪接线）。

### 轮尾改动文件卡片（2026-09-21，与官方 web 端同步）

- **每个顶层轮次结束后，助手消息尾部显示本轮的改动文件卡片**：标题给 Host 报的
  完整文件数与增删行数合计，每行是该文件的展示路径与 `+a −d`（二进制显示「二进制」、
  Host 没捕获内容的显示「过大」），折叠前 3 行、下方可展开/收起。数据来自 Host 的
  认证路由 `GET /api/changes.summary`（官方 `ui-deliverables` 的同一份契约，见
  `docs/dsh-server-api.md` 的「改动清单路由」）——清单**不在**会话日志里：
  `workspace/changes` 事件只带轮号，清单与逐文件对比留在 Host 内存中，所以 Host
  重启过、或 Session 已释放（打开旧会话）时**没有卡片**，退回原来那行「本轮文件改动」。
- **卡片显示出来时，「本轮文件改动」文件行让位**（同一个文件不说两遍）；清单还没问到
  或不存在时那一行照旧——信息不会两头都丢。
- **`workspace/changes` 归入「渲染」名单**：此前它在名单之外，而内核 0.1.6-alpha 起
  每个顶层轮次都会追加一条，于是每轮结束都弹一次「本客户端不认识的事件」（见
  `scripts/unknownEvent.test.ts` 的回归点）。
- 行点击与文件芯片同一套语义（默认看改动、按住 Alt 直接打开文件），卡片标题在第一个
  文件上打开它；清单按 `(sessionId, seq)` fetch-once 缓存、换连接整体作废。
  断言 `scripts/changesCard.test.ts`（形状校验 / 折叠 3 行 / 缓存键带会话 id /
  适配器折叠坐标 / 归约只收当前会话 / 两端接线）。

**用户 2026-09-21 反馈后的三处修正**（同一功能，未单独发布）：

- **`+绿 −红`**：增删行数拆成两个节点分别上色，颜色取 diff 视图同一对变量
  （`--success` / `--error`）。注意 `.diff-stat` 那两条规则**限定作用域**，光复用
  `.diff-add` / `.diff-del` 类名只会是灰的（第一版实测 `rgb(140,140,140)`），
  所以卡片自己显式给了一遍——断言钉住了这个坑。
- **让位判定改成按轮**：一轮被插话切成多段时（`a:N` / `a:N:2`…），卡片挂在最后一段、
  `produced` 挂在前一段，按消息判定会让「卡片」与「本轮改动」在同一轮尾部并排
  （用户报的「一轮尾部三样一起出现」）。现在由 App 用 `turnsWithChangesCard` 算出
  「哪些轮真的有卡片」再传给消息。
- **首帧从半轮开始时的轮号归位**：`session/follow` 只带最近 60 条，边界落在轮中间时
  还没有 `turn/start`，这一轮的内容会挂到凭空建出的 `a:0` 上；往上翻、加载更早的
  历史后重折，内容归并回 `a:N`——界面上就是「同一轮先显示成两条消息、加载全历史后
  又少一条」。现在带 `turn` 的事件（`assistant/message`、`step/*`、`tool/*`、
  `system/message`）在**当前轮号未知**时会把轮号补上；`command/run` 这类轮前事件不带
  `turn`，`a:0` 的幻影轮语义照旧。
- 取证工具：`scripts/tailRowsProbe.ts`（`node build/tail-rows-probe.mjs --ws <工作区> --all`）
  离线回放真实会话日志，对拍「首帧」与「加载全部历史」两条折叠路径，零 token。
  第一版它自己用错了 API（`applyEvent` 不会 `remember`，重折只认 `seen`），
  误报出「交付与卡片丢失」——已修正为走 `applyFrame`（与线上同一条路径）。
- **已知且有意**（用户 2026-09-21 口径：保持现状）：Host 已释放那份清单的轮次
  （Host 重启过、Session 早已 dispose）没有卡片数据，那一轮**退回「本轮改动」文件
  行**而不是显示空卡片——信息不丢，代价是这类老轮次与新轮次的形态不一样。
  要消除这个差异只能自己再算一份改动清单（宿主侧 git 比对 + `produced` 推导），
  口径会与官方不同，暂不做。

### 未知事件进日志：一闪而过的提示条现在可追溯（2026-09-21）

- **每种「本客户端不认识的事件」都会在宿主输出通道留下首见一条**（`adapter.ts` 的
  `noteUnknownEvent`；控制器注入 `log` 并带会话 id）：类型、seq、data 摘要（紧凑
  单行，超 600 字符截断）以及它在界面上的处置（提示用户 / 按 `ignorable` 静默跳过）。
  此前这类事件只有一条几秒后消失的 toast，用户回报时现场已无从查起；协议文档
  （docs/dsh-server-api.md §6.1）要求的降级纪律本来就写着「至少在输出通道里报一次」。
- **按类型去重**：`refold()` 会把已记录的事件整体重折（跟随流开窗、socket 重连、
  加载更早的历史），同一类型会重放很多遍——日志长度只跟「内核新增了几种词汇」有关，
  而不是跟它出现了多少次有关（一次会话几十轮 `workspace/changes` 只占一行）。
  `ignorable` 的未知事件界面上完全无声，但同样留痕（排查线索，不该打扰用户）。
  断言 `scripts/unknownEvent.test.ts` 第 6 组。
...
- **跑完的子代理读作「已完成」**（绿灯），不再读作「未运行」：`subagents/list` 的
  `activity` 只有 `running` / `inactive` 两档，而列在目录里的子代理必然领过初始任务
  （one-shot 与 continuable 都是带着 prompt 建出来的），所以「不在跑」就是跑完了。
  文案 `subagentInactive`（未运行）随之删掉，换成 `subagentCompleted`。
- **子代理行的状态点独享一格**：格内水平垂直都居中，自带左外边距把它与标题隔开
  （此前是紧跟在标题末尾的裸圆点，行 gap 只有 2px）。`.row-icon-status` 那个工具行
  坑位也补上了 `align-items: center`（同一类「独享空间必须居中」）。
- **右上角两颗面板入口会在有东西在跑时亮起并呼吸**：子代理按钮吃「目录里 `activity:
  running` ∪ 后台任务里 `kind: 'subagent'` 的活行」，后台任务按钮吃任意活行；
  判据是纯函数 `webview/activity.ts`（断言 `scripts/activity.test.ts`），
  呼吸复用 `.dot-running` / `.icon-glow` 那组关键帧（节奏必须一致）。
- **点进子代理能看它的会话记录**：抽屉**立即**开到这个 id（带「正在读取子代理会话…」
  占位），宿主那份只读快照到了即填入，并**默认停在最新一条**；记录**只读**——
  问卷 / 审批卡只画内容（选项 disabled、无输入框、无提交行），面板里没有任何输入区。
  顺带修掉两个静默坑：宿主在目录里查不到那个 id 时不再**什么都不回**（界面会永远停在
  「正在读取」）；`ui/openSubagent` 会先清掉上一次的记录（先点 A 再点 B 不再拿 A 充数）。
- **问卷的自定义回答：点编辑区也算选中它**（用户 2026-09-19 报）——此前 `onMouseDown`
  对 `.question-input` 直接放行，点进去只有打字才选中。现在先补**选中**（只置真、不切换，
  Focus 与 MouseDown 前后脚到也不会翻回去）再放行给输入框；焦点进编辑区（Tab）同理。
- **分支会话与普通会话同级**（不再缩进）：靠标题前缀「分支: 原会话标题」区分。
  随之删掉只服务于缩进的 `depth` / `lineageDepths`（宿主、线格式、样式、夹具、断言一并清掉）。
