...
// 7.12 agentPreset：`string | null`，空串与 null 同义（= 这个部署没有组装预设）
{
  ...
  // modeSelectionEnabled}`；0.1.7-rc.2 起只剩 `{presets}`，可见性判据改由客户端偏好
  // （第二个参数）给。两条判据都只有一处实现（`agentPresetsFromList`），界面不再自己判。
  assert.deepStrictEqual(
    agentPresetsFromList({
      presets: [
  ...
  // 0.1.7-rc.2 起 roster 里没有这个策略：客户端偏好说了算
  const newRoster = { presets: [{ id: "standard", isDefault: true }] };
  assert.deepStrictEqual(
    agentPresetsFromList(newRoster),
    { options: [{ id: "standard", isDefault: true }], selectable: true },
    "缺省（读不到客户端偏好）按允许——官方 schema 的默认值就是 true",
  );
  assert.deepStrictEqual(
    agentPresetsFromList(newRoster, true),
    { options: [{ id: "standard", isDefault: true }], selectable: true },
  );
  assert.deepStrictEqual(
    agentPresetsFromList(newRoster, false),
    { options: [], selectable: false },
    "客户端关掉选择入口 = 目录给空表",
  );
  // 老服务端的显式策略压过客户端偏好（两代都读，新对话不改变旧行为）
  assert.deepStrictEqual(
    agentPresetsFromList({ presets: [{ id: "standard", isDefault: true }], modeSelectionEnabled: false }, true),
    { options: [], selectable: false },
    "服务端说不许选就不许选，客户端偏好改不了它",
  );
  assert.deepStrictEqual(
    agentPresetsFromList({ presets: [{ id: "standard", isDefault: true }], modeSelectionEnabled: true }, false),
    { options: [{ id: "standard", isDefault: true }], selectable: true },
  );
  // 策略字段形状不对（不是布尔）按「服务端没表态」处理，落回客户端偏好
  assert.deepStrictEqual(
    agentPresetsFromList({ presets: [{ id: "standard" }], modeSelectionEnabled: "yes" }, false),
    { options: [], selectable: false },
  );
  ...
  // 形状认不出（没给目录 / 坏值）一律按「没有」处理：谈不上允许不允许，不拿客户端偏好去猜
  ...
  assert.deepStrictEqual(agentPresetsFromList(undefined, true), { options: [], selectable: false });
}
console.log("projections: agentPreset 投影 + roster 目录（坏预设与选择可见性两代判据）✓");
