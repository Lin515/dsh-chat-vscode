...
export const BAR_ORDER: readonly {
  ...
  // P2 末位（用户口径）：模型胶囊的升级档——`提供商/模型名` 全格式。宽度充裕时
  // 才从裸模型名（rank 1 的 pinned 档）升上来；它是 model 槽位的第二档，rank 必须
  // 大于同槽位的第一档（算法按 rank 逐档升级，不递增就升不上去）。
  { slot: "model", level: "provider", rank: 10 },
];
