...
### 3.7 **守护进程与 dsh 同生共死**（所以协议里没有「启动后台」控制帧）

用户 2026-09-19 的设计口径：**守护进程存在的唯一理由是端着一套可用的 dsh 后台**——
它启动就无条件把 dsh 拉起来，dsh 崩了就重起；反过来，dsh 不该在守护进程还活着时缺席，
守护进程也不该在 dsh 已经不需要时独自赖着。这条不变量落成三句话：

1. **起来就自启**：监听之后就 `await bringUp()`，不等任何窗口连上（`main.ts`）；
2. **崩了就重起**：主循环（≤1s 一跳）发现 `childGone()` 且**还有活连接** → `bringUp()`。
   零客户端时不救——按上面的原则，那种情况下这一对本来就该退场（空闲判定随即收场）。
   顺带这也是**防 spawn 风暴**：命令写错时 dsh 根本起不来，若零客户端也无限重起，
   只会把日志刷爆并拖住本该退场的守护进程；
3. **收场就一起收**：`stop`（用户点「停止内部 DSH」）与 `idle`（空闲超阈值）都走
   `shutdown()` → 先 `killServer()` 杀掉 dsh 整棵树，再 `goodbye()`、自己退出。

**因此控制协议里没有也不需要「启动 dsh 后台」这一帧**：客户端的控制动作只有
`restart` 与 `stop`（见 `dsh/supervisorWire.ts` 的 `ClientMessage`）。扩展侧的
`ensure({start:true})` 是"**守护进程不在就把它拉起来**"，不是"叫守护进程去起 dsh"；
dsh 的启动/重启/收场**只有守护进程会做**。以后别为了"更灵活"加一个 start 帧，
也别让守护进程能不绑 dsh 地活着——那正是这条不变量要挡住的两件事。
...
> "内部后台"这一种情形这条不变。
>
> **2026-09-19 修正**：下面这句原本写的是"`target === "external"` 时管理器的内部那套逻辑
> 一步都不跑（心跳、`restart()`、socket 全部门在目标判据后面）"——**前半句对"决策"成立，
> 对"手里那条 socket"不成立**：换目标时它照旧开着，于是守护进程一直把 dsh 拎着、状态推送
> 也继续改写管理器状态。修法与不变量见 **§9.8**。
...
| 用户点「停止连接」 | 停掉**正在进行的连接**（中止在途那一轮 + 关掉自动重连），并**交还内部后台的占用**（断开与守护进程的连接、心跳不许再接回，见 §9.9）；**进程一个都不动**（杀 dsh 永远是守护进程的事），条上给按钮态 |
...
探测频率：**随 5 秒心跳刷新**，两者都不与守护进程"通讯"。

- **内部那一轴**：读会合文件 + `isProcessAlive(supervisorPid)`（`process.kill(pid, 0)`）——
  纯只读，对方不参与；它的唯一用途是按钮态那句"内部 DSH：运行中/未运行"；
- **外部那一轴**：一次 HTTP GET，且**只在未连接时做**（连上之后连接条根本不显示，"连上了"
  本身就是可达证据）。已连上时**沿用上一次的结论**，不改写成"不可达"——那是假的，我们正连着它。
  这道门槛 2026-09-19 才真正落实在 `probeFacts()` 内部（此前注释这么写、调用方却没按它调，
  于是连上之后每 5 秒仍会朝那个地址发一次 GET）。
...
| 「连接内部 DSH」 | internal | **true**（有就接上、没有就起一套——**与「启动内部 DSH」同一套逻辑**，见 §9.4 的说明） |
...
**两轴探测的结论**，而它与后台的真实状态之间必然有偏差（守护进程刚退场、另一个窗口刚拉起、
探测的 5 秒节拍），于是会出现"显示启动、其实已经在跑"或"显示连接、其实还没起"。让两者语义
相同，用户点哪个都能把内部后台用起来，也就不会出现"点对了按钮却什么都没发生"。
拉起这条路**不需要扩展额外发指令**：守护进程自己起来就 `bringUp()` 把 dsh 拉起
（`supervisor/main.ts`），扩展只要连上它的 socket 并等就绪（`bringUpWith`）。
`autoConnect` 与这条无关——它只决定**激活期要不要自动连**，不拦任何用户点击（见 AGENTS.md 同一节）。
...
- **`prepareRound()`** = 作废在途轮（`connectRoundId + 1`）+ `cancelWaiting()` +
  ...
  其中「停止连接」在这之上还要**交还内部后台的占用**（`releaseInternal()`，见 §9.9）：
  `prepareRound` 收的是与 **dsh** 的连接，守护进程那条 socket 归 `releaseInternal` 管。
...
### 9.8 换目标是**彻底**的：切到外部就断开内部那条 socket（2026-09-19 修）

用户实测：本来是内部那套，内部 dsh 崩掉（守护进程又把它拉起来）之后，用户点
「连接外部 DSH」连上了外部——但守护进程自动重启的那个内部 DSH **一直存在**，
诊断信息里显示的也还是内部 DSH。

根因是**换目标只换了"决策"，没换"手里握着的东西"**：`ensure({target:"external"})` 改了
`target`、等外部地址、报就绪，而那条通往守护进程的常驻 socket **一直开着**。两个后果：

| 后果 | 机理 |
|---|---|
| **内部 dsh 永远不退场** | 守护进程的空闲判据就是"socket 上还有没有活连接"（§3.5 ①）。本窗口那条连接一直挂着，它就一直认为"还有人用"，于是不断把崩掉的 dsh 拉回来 |
| **诊断显示"用的是内部"** | 那条连接照旧收 `state` 推送，`onStatePush` 把 `status` 改回内部那一套（地址、进程、共用窗口数），而诊断命令读的正是 `status` / `peekState()` |

现在的纪律一句话：**目标不是内部 ⇒ 与守护进程之间没有任何连接、也没有任何内部状态记忆**。

- **`SupervisorManager.detachInternal(reason)`** 是唯一落点：关连接 + 清 `state` + 清
  `launched`/`clientCount` + 作废指向内部那份 `status.info`。**断开不碰任何进程**——
  内部后台退不退场由守护进程按"还有没有活连接"自己裁决（没有别的窗口连着 → 空闲阈值后
  连 dsh 一起收场）。这正是"换到外部之后内部该消失、别的窗口还在用就该继续"的正确答案；
- **三个地方各断一次**（不靠单一入口的记忆）：`ensure()` 的外部分支（换目标的主路径）、
  5 秒心跳（兜底，覆盖"还没连上那一轮"、"`autoConnect` 关掉"这类路径）、以及
  **连接回调按目标守卫**（`onState`/`onGoodbye` 都要求"这条连接还是当前的、且目标是内部"
  才写状态——收尾帧可能正好在断开过程中到达，它若照旧写，界面会被推回"内部退场/换地址"）；
- **`peekState()` 不再回落到读磁盘**：从前 `this.state ?? readState(...)` 会把磁盘上那份
  （别的窗口在用的、或正在收场的）内部后台当成"本窗口的后台"报出来（诊断里那行
  "后台进程：守护进程 X, dsh Y" 就是它）；
- 控制器那侧两道闸：`onServerStatus` 在目标为外部时**提前返回**（内部那套的退场/失败不许
  改写界面状态），`stopServer` 在目标为外部时**不 `prepareRound()`**（外部那条客户端 +
  跟随流一个字都不动，界面也不被拉回按钮态）；
- **「停止内部 DSH」在分离态下短暂接入**（`stopDetachedInternal`：不写进 `this.connection`、
  `onState` 是空实现，发完 stop 就断）——不接入的话这条命令只能记一条日志，而命令面板上
  它还写着「停止内部 DSH」。回执按"请求到底有没有发出去"给：内部守护进程不在时如实说
  「内部 DSH 没有在运行」，不再谎报"已停止"。

**诊断的连带改动**：多一行「当前连接目标：内部 DSH（守护进程）/ 外部 DSH（dshChat.url）/
尚未选定」。内部优先之后，"地址 + 是不是本扩展启动的"已经不足以回答"我现在连的是哪一个"，
而这恰恰是用户打开诊断要问的第一个问题。

**验证**（`supervisorPolicy.test.ts` 第 7 组用**真的** socket 服务端数活连接，不读源码）：
接内部（对面 1 条）→ 切外部（对面必须 **0** 条、状态仍是外部、`peekState()` 为空）→
再切回内部（对面重新 1 条）；控制器那两道闸由 `connectionStop.test.ts` 第 5 组按源码结构钉住
（第 6 组钉上面那道兜底的**顺序**——写反了它永远不生效，而别的断言都不会红）。
端到端：`node build/supervisor-manager-probe.mjs` 第 6 步用**真 supervisor + 真 dsh** 走一遍
用户报的那个场景——连内部 → 切外部 → 内部后台自己退场（会合文件消失、dsh 端口关闭）。

> 测试写法上的一条实测事实：服务端 socket **必须挂了 `data` 监听**才会收到对端 `destroy()`
> 的 `close`——paused 的 socket 不跑读循环，2 秒后依然 `closed=false`（本机实测对照：
> flowing 立刻 true）。真实守护进程本来就挂了 `data`（解析 control 请求），假服务端要照它写，
> 否则数的不是同一件事。

### 9.9 「停止连接」= **不再占用内部后台**（2026-09-19 用户口径）

口径：**不连就不占用**。此前「停止连接」只收掉与 **dsh** 的连接（客户端 + 跟随流）与重试
循环，守护进程那条 socket **留着**——于是它永远认为"还有人在用"：内部 dsh 不会按空闲退场，
而它的推送还能把界面从"已停止"拉回"连接中/错误"。更糟的是那条 socket 掉了之后，心跳会
`readState` + `isProcessAlive` 再 `connect()` 把它**接回来**（接入分支排在 `stoppedByUser`
检查之前，那个标志只管"不许拉起"，挡不住"接上已经在跑的"）。

现在两件事必须同时做（少任何一件都等于没做）：

| 动作 | 落点 | 少了它的后果 |
|---|---|---|
| **断**：关掉与守护进程的连接 | `SupervisorManager.releaseInternal()` → `detachInternal()` | 守护进程一直把本窗口算作"还有人用"，内部 dsh 永不空闲退场 |
| **挡**：置位 `detachedByUser`，心跳**不许**自动接回 | `heartbeatTick` 里那道必须排在 `connection.connected` **之前** | 断开后 5 秒，心跳发现"会合文件里那套还活着"又接上了——用户看到"点了停止，过一会儿又连上了" |

配套的三处（不一致就会露馅）：

- **`bringUp()` 清掉 `detachedByUser`**：它是"有人**显式**要这套后台"的通行证（发消息 /
  三个连接按钮 / 重启 / 激活期选路）。少了它，用户叫停之后就再也连不回来了；
- **`abandonRound()` 再 release 一次**：一轮**排队/重试**的连接可能在叫停之后才走到
  `bringUp`（那会清掉标记）并在管理器侧把连接建起来，而控制器随后因为"用户叫停"把它放弃
  ——连接就留在了管理器手里。幂等，所以无脑再收一次；
- **`peekState()` 在 `detachedByUser` 时返回 undefined**：占用已经交还，它随时会退场，
  这时再报它的 pid 就是在说"我在用它"（诊断命令读的就是这里）。

**代价说清楚**：内部后台真退场之后，下一次发消息走"拉起一套"（冷启动 5~8 秒）；别的窗口
还在连着时它继续服务——那正是正确的结果。进程**一个都不动**这条纪律不变：退不退场由
守护进程按"还有没有活连接"自己裁决。

> 顺带修掉一个判据错误：`stopAndExit()`（「停止内部 DSH」）原来按**目标是不是内部**决定要不要
> 短暂接入，现在按**手里有没有活连接**。按目标判会把"交还占用后守护进程仍在空闲窗口里活着"
> 误当成"没有可停的东西"，回执谎报「内部 DSH 没有在运行」——而它明明在跑。

**交还之后再点「重启内部 DSH」也必须真的重启**（同一次修）。这条路径在停止连接之后是常态，
而 `restart()` 原来在"连接不在"时直接 `return this.ensure(...)`：**只接上、一个控制帧都没发**，
控制器与扩展却照旧弹"DSH 服务器已重启。"。现在先接上、再照旧发 `restart`；同时把
`previous = this.state?.serverPid`（用来判"supervisor 是不是还没开始重起"）挪到**接上之后**
——早读时 `this.state` 是空的，`previous` 为 undefined，那句 `state.serverPid === previous`
永远不成立，于是会把**旧地址**当成"重启完成"报出去（症状：令牌没变、地址照旧）。

**回答"停止连接之后还能不能连上这个守护进程"**：能，但**只由用户显式动作触发**——「连接内部
DSH」/「启动内部 DSH」（这两个是**同一套逻辑**：有就接上、没有就起一套，见 §9.4）、
发消息 / 新建 / 切会话、「重启内部 DSH」，以及**重载窗口或新开窗口**（新管理器不继承这两个标记，
按激活期选路自动接上）。自动路径（5 秒心跳）**不会**替你接回去——那正是 `detachedByUser` 存在的意义。

**验证**：`supervisorPolicy.test.ts` 第 8 组（真 socket 数活连接：接内部 → `releaseInternal()`
对面 0 条 → **调一次心跳，对面仍是 0 条** → 显式动作能重新接上 1 条）；第 9 组（交还之后再
`restart()`：假守护进程必须**收到 `restart` 控制帧**，且返回的是**新地址**而不是接上时那份旧的
——假守护进程刻意拖 700ms 才写新状态，就是为了让"`previous` 读早了"那种写法显形）；
